...
async function saveToFiles(lounges, isIntermediate = false) {
  ...
  // 保存JSON格式
  const jsonFile = path.join(outputDir, `all-lounges${suffix}.json`);
  fs.writeFileSync(jsonFile, JSON.stringify(lounges, null, 2), "utf8");
  console.log(`💾 JSON数据已保存到: ${jsonFile}`);

  // 保存CSV格式
  const csvFile = path.join(outputDir, `all-lounges${suffix}.csv`);
  const csvHeader =
  ...
  // 保存Excel格式
  const excelFile = path.join(outputDir, `all-lounges${suffix}.xlsx`);
  ...
  const codesFile = path.join(outputDir, `all-lounge-codes${suffix}.json`);
  fs.writeFileSync(codesFile, JSON.stringify(loungeCodes, null, 2), "utf8");
  console.log(`💾 贵宾厅代码列表已保存到: ${codesFile}`);

  // 保存贵宾厅代码Excel
  const codesExcelFile = path.join(outputDir, `all-lounge-codes${suffix}.xlsx`);
